...
from aegisforge.llm.providers import ModelProvider, get_model_provider
...
def plan_node(state: dict[str, Any]) -> dict[str, Any]:
    ...
    if _ctx.model_provider is not None:
        planner = LLMPlannerAgent(model_provider=_ctx.model_provider)
...
def execute_agent_node(state: dict[str, Any]) -> dict[str, Any]:
    ...
    if agent_type == "rag" and _ctx.retrieval_service is not None:
        agent = RAGAgent(retrieval_service=_ctx.retrieval_service)
...
def build_execution_graph() -> StateGraph:
    ...
    graph.add_node("retry_or_complete", retry_or_complete_node)
    ...
    if resume_from_checkpoint and checkpointer is not None:
        ...
        if resumed_state is not None:
            ...
            if resumed_state.get("status") == RequestStatus.ACTION_REQUIRES_APPROVAL.value:
                resumed_state["status"] = RequestStatus.EXECUTING.value
...
def resume_workflow_after_approval(
    ...
    # Approval granted — reset status to executing and continue
